...
def find_dicom_files(patient_id):
    ...
    Retorna: (femoral_dcm, femoral_jpeg, forearm_dcm, forearm_jpeg, hip_side)
    ...
    return femoral_dcm, femoral_jpeg, forearm_dcm, forearm_jpeg, hip_side
...
def extract_from_xml(xml_text):
    ...
    # IMPORTANTE: La ubicación de los datos varía según el tipo de escaneo:
    # - Para HIP: Neck está en fila 1, Total en fila 2
    # - Para LSPINE: L1-L4 en filas 1-4, Total en fila 5
    # Columna 3: BMD, Columna 4: T-score, Columna 6: Z-score
    
    # Buscar patrones tipo: ResultsTable1[ 1][ 3] = "0.691";
    # Estrategia: Buscar en TODAS las filas y detectar por nombre de región
    
    # Buscar FEMORAL (puede ser "Neck", "Femoral Neck", etc.)
    for row in range(10):
        ...
        if region_match:
            ...
            if 'NECK' in region_name and 'FEMORAL' not in region_name:  # Es "Neck" solo
                ...
                # NUEVA REGLA: Si hay múltiples regiones (Total y Neck), buscar la fila correspondiente a Neck
                ...
                first_region_match = re.search(r'ResultsTable2\[\s*1\]\[\s*0\]\s*=\s*"([^"]+)"', xml_text)
                ...
                if has_region_column:
                    ...
                    for hist_row in range(1, 20):
                        region_name_match = re.search(rf'ResultsTable2\[\s*{hist_row}\]\[\s*0\]\s*=\s*"([^"]+)"', xml_text)
                    ...
                    if history_row is not None:
                        test_change = re.search(rf'ResultsTable2\[\s*{history_row}\]\[\s*5\]\s*=\s*"([^"]+)"', xml_text)
                    ...
                    # Formato simple sin columna de región
                    history_row = 1
                ...
                change_match = re.search(rf'ResultsTable2\[\s*{history_row}\]\[\s*6\]\s*=\s*"([^"]+)"', xml_text)
                if not change_match or not change_match.group(1).strip() or change_match.group(1).strip() == ' ':
                    change_match = re.search(rf'ResultsTable2\[\s*{history_row}\]\[\s*5\]\s*=\s*"([^"]+)"', xml_text)
                
                # Extraer fecha previa - priorizar columna 0 (fecha) sobre columna 1
                prev_date_match = re.search(rf'ResultsTable2\[\s*{history_row + 1}\]\[\s*0\]\s*=\s*"([^"]+)"', xml_text)
                ...
                if not prev_date_match or not prev_date_match.group(1).strip() or '/' not in prev_date_match.group(1):
                    prev_date_match = re.search(rf'ResultsTable2\[\s*{history_row + 1}\]\[\s*1\]\s*=\s*"([^"]+)"', xml_text)
                
                # Extraer BMD previo - puede estar en columna 2 o 3
                prev_bmd_match = re.search(rf'ResultsTable2\[\s*{history_row + 1}\]\[\s*3\]\s*=\s*"([^"]+)"', xml_text)
                if not prev_bmd_match or not prev_bmd_match.group(1).strip():
                    prev_bmd_match = re.search(rf'ResultsTable2\[\s*{history_row + 1}\]\[\s*2\]\s*=\s*"([^"]+)"', xml_text)
    ...
    if is_hip_study:
        ...
        # Intentar extraer FRAX con prior fracture (columna 2 en ResultsTable3)
        # Esto aparece cuando el paciente tiene historial de fracturas previas
        if is_frax_table3:
...
def generate_report(data):
    ...
    if has_left_forearm and has_right_forearm:
    ...
    # FRAX - solo mostrar valores principales (sin "with prior fracture")
...
"""
    ...
    if lumbar_prev_date:
        comparison_text = lumbar_prev_date
    ...
    if has_lumbar:
        ...
        lumbar_section = f"<strong>LUMBAR SPINE:</strong> The bone mineral density in the lumbar spine (L1-L4) is {lumbar_bmd} g/cm² with a T-score of {lumbar_tscore}{zscore_text}.{lumbar_comparison}\n"
    ...
    try:
        ...
        if existing:
            ...
            for key, value in existing_data.items():
                ...
                # Para FRAX "with prior fracture", NO preservar valores antiguos si no vienen en datos nuevos
                # Estos deben venir explícitamente del XML cuando existen
                if key in ['major_fracture_risk_prior', 'hip_fracture_risk_prior']:
                    # Solo usar datos nuevos, no preservar antiguos
